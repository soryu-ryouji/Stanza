...
public static class StanzaWriter
{
    ...
    /// <summary>供 GUI 编辑器的可编辑主行：仅描述。优先级/项目/标签/截止日都是结构化属性，
    /// 不以文本记号形式常驻编辑框（§7.2.1/§7.2.4/§7.2.5；截止日经右侧着色文本展示）。</summary>
    public static string ComposeEditableHeader(StanzaTask task) => task.Description ?? "";
