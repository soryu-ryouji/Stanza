...
1. **解析严格遵循 RFC 1.5.0**：四状态区块、主行元数据（`(A)`–`(D)` 四象限优先级、`YYYY-MM-DD` 截止、`+项目`、`#标签`）、缩进续行、备注内空行、时间戳属性块（§7.4.3）。GUI 的编辑文本不含优先级前缀：优先级是 ViewModel 上的结构化属性（菜单/工具栏设置），写出时由 `ToModel` 重组。代码注释中用 `§x.y` 引用 RFC 条款，新增规则时保持这个习惯
