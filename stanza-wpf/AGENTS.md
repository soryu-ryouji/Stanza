...
Stanza 是 [Stanza 纯文本任务管理格式](https://github.com/Ryougi-Shiki0/Stanza)（RFC 1.4.0）的 Windows 桌面编辑器。WPF + MVVM，零第三方依赖，目标框架 .NET 10。
...
1. **解析严格遵循 RFC 1.4.0**：四状态区块、主行元数据（`(A)`–`(D)` 四象限优先级，可附加单位数象限内序号如 `(A3)`、`YYYY-MM-DD` 截止、`+项目`、`#标签`）、缩进续行、备注内空行、时间戳属性块（§7.4.3）。代码注释中用 `§x.y` 引用 RFC 条款，新增规则时保持这个习惯
...
5. **DOING / WAIT 自动排序**：按（优先级象限 → 象限内序号 → 截止日期）稳定排序；拖拽只调整同优先级内的相对顺序
